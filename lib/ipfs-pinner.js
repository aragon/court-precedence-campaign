...
function requiresIpfsUpload(justification) {
  return !justification.startsWith('ipfs:') && justification.endsWith('.md')
}
...
async function ipfsUpload(disputePath, justification, sender) {
  ...
  if (justification.endsWith('.md')) {
    ...
    if (!fs.existsSync(justificationPath)) throw Error(`Justification path "${justificationPath}" does not exit`)
